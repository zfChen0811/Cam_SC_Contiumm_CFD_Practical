...
int main(int argc, char *argv[]) {
	...
	u1_pri.resize(nCells + 4,vector<double>(3));

	std::vector<std::vector<double>> u1_con;
	u1_con.resize(nCells + 4,vector<double>(3));
	
	std::vector<std::vector<double>> u2_pri;
	u2_pri.resize(nCells + 4,vector<double>(3));
	
	std::vector<std::vector<double>> u2_con;
	u2_con.resize(nCells + 4,vector<double>(3));
	
	std::vector<double> w1x;
	w1x.resize(3,0);
	
	std::vector<std::vector<double>> delta1x0;
	delta1x0.resize(nCells + 4,vector<double>(3));
	
	std::vector<std::vector<double>> delta1x1;
	delta1x1.resize(nCells + 4,vector<double>(3));
	
	std::vector<std::vector<double>> delta1x;
	delta1x.resize(nCells + 4, vector<double>(3));
	
	std::vector<double> w2x;
	w2x.resize(3,0);
	
	std::vector<std::vector<double>> delta2x0;
	delta2x0.resize(nCells + 4,vector<double>(3));
	
	std::vector<std::vector<double>> delta2x1;
	delta2x1.resize(nCells + 4,vector<double>(3));
	
	std::vector<std::vector<double>> delta2x;
	delta2x.resize(nCells + 4, vector<double>(3));
	...
	for(int i = 2; i < nCells + 2; i++){
		double x = x0 + (i - 1.5) * dx;
		if(x <= 0.5){
			u_pri[i][0] = u1_pri[i][0];
			u_pri[i][1] = u1_pri[i][1];
			u_pri[i][2] = u1_pri[i][2];
			phi[i] = x - 0.5;
		}else if(x > 0.5){
			u_pri[i][0] = u2_pri[i][0];
			u_pri[i][1] = u2_pri[i][1];
			u_pri[i][2] = u2_pri[i][2];
			phi[i] = x - 0.5;
		}
	}
	...
	do{
		for(int k = 0; k < 3; k++){
			if(k == 0 || k == 2){
				u1_pri[1][k] = u1_pri[2][k];
				u1_pri[0][k] = u1_pri[3][k];
				u1_pri[nCells + 2][k] = u1_pri[nCells + 1][k];
				u1_pri[nCells + 3][k] = u1_pri[nCells][k];
				
				u2_pri[1][k] = u2_pri[2][k];
				u2_pri[0][k] = u2_pri[3][k];
				u2_pri[nCells + 2][k] = u2_pri[nCells + 1][k];
				u2_pri[nCells + 3][k] = u2_pri[nCells][k];
			}else if(k == 1){
				u1_pri[1][k] = -u1_pri[2][k];
				u1_pri[0][k] = -u1_pri[3][k];
				u1_pri[nCells + 2][k] = -u1_pri[nCells + 1][k];
		...
		phi[0] = phi[2];
		phi[1] = phi[2];
		phi[nCells + 2] = phi[nCells + 1];
		phi[nCells + 3] = phi[nCells + 1];
		...
		x1L.resize(nCells + 4,vector<double>(3));
		
		std::vector<std::vector<double>> x1R;
		x1R.resize(nCells + 4,vector<double>(3));
		
		std::vector<std::vector<double>> x2L;
		x2L.resize(nCells + 4,vector<double>(3));
		
		std::vector<std::vector<double>> x2R;
		x2R.resize(nCells + 4,vector<double>(3));
		...
		u1_bar.resize(nCells + 4,vector<double>(3));
		
		std::vector<std::vector<double>> half1xL;
		half1xL.resize(nCells + 4,vector<double>(3));
		
		std::vector<std::vector<double>> half1xR;
		half1xR.resize(nCells + 4,vector<double>(3));
		
		std::vector<std::vector<double>> u2_bar;
		u2_bar.resize(nCells + 4,vector<double>(3));
		
		std::vector<std::vector<double>> half2xL;
		half2xL.resize(nCells + 4,vector<double>(3));
		
		std::vector<std::vector<double>> half2xR;
		half2xR.resize(nCells + 4,vector<double>(3));
		...
		flux1_x.resize(nCells + 3,vector<double>(3));
		
		std::vector<std::vector<double>> flux2_x;
		flux2_x.resize(nCells + 3,vector<double>(3));
