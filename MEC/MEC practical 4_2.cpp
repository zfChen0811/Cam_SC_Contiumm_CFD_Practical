...
double pref(double rho);
double eref(double rho);
...
double pref(double rho){
	...
	double v0 = 1 / rho0;
	double v = 1 / rho;
	double p = A * exp(-(R1 * v)/v0) + B * exp(-(R2 * v) / v0) + C * pow((v/v0), (-GAMA - 1));
	return p;
}
...
double eref(double rho){
	...
	double v0 = 1 / rho0;
	double v = 1 / rho;
	double e = -de + v0 * ((A / R1) * exp(-(R1 * v) / v0) + (B / R2) * exp(-(R2 * v) / v0) + (C / GAMA) * pow((v/v0), (-GAMA)));
	return e;
}
...
std::vector<double> con2pri(std::vector<double>con, double gama){
	...
	pri[0] = con[0];
	pri[1] = con[1] / con[0];
	pri[2] = pref(con[0]) + DELTA * pri[0] * (e - eref(pri[0]));
	return pri;
}
...
int main(int argc, char *argv[]) {
	...
	do{
		for(int k = 0; k < 3; k++){
			if(k == 0 || k == 2){
				...
				u_pri[nCells + 3][k] = u_pri[nCells][k];
			}else{
				u_pri[1][k] = -u_pri[2][k];
				u_pri[0][k] = -u_pri[3][k];
				u_pri[nCells + 2][k] = -u_pri[nCells + 1][k];
	...
	for(int i = 2; i < nCells + 2; i++){
		outFile << x0 + dx * (i - 0.5 - 2) << " " << u_pri[i][2] << endl;
	}
	outFile.close();
}